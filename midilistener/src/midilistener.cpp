...
static const char *DEFAULT_TOPIC_SPANNED = "music-spanned";
...
bool parseOptions(int argc, char *argv[], std::string &mqttServer,
                  unsigned int &mqttPort, std::string &mqttTopicInstant,
                  std::string &mqttTopicSpanned, std::string &clientName,
                  bool &debug);
...
int main(int argc, char *argv[])
{
    ...
    std::string mqttTopicSpanned;
    bool debug;

    if (!parseOptions(argc, argv, mqttServer, mqttPort, mqttTopicInstant,
                      mqttTopicSpanned, clientName, debug))
    {
        return 0;
    }
    ...
    try
    {
        ...
        MusicSensor<bsf::AsyncMqttTransport> sensor(
            transport, mqttTopicInstant, mqttTopicSpanned, clientName);
